...
if IsValid(Wep:GetHoldType()) and (dmginfo:IsExplosionDamage() or (dmginfo:IsDamageType(DMG_SLASH + DMG_CLUB)) and (table.HasValue(MeleeTypes, Wep:GetHoldType()) or Wep:GetSlot() == 0)) then
...
if IsValid(Wep:GetHoldType()) and (Wep:GetClass() == "weapon_fists" or Wep:GetHoldType() == "fist") then
	DMGBuff = DMGBuff + 0.15
end
end
...
if PlyHasPerk(Atk, "20 ") == true and IsValid(Wep) and IsValid(Wep:GetHoldType()) and ((Wep:GetHoldtype() == "revolver" and Wep:GetPrimaryAmmoType() == "357") or Wep:GetHoldType() == "knife" or Wep:GetPrimaryAmmoType() == "Buckshot") then
	DMGBuff = DMGBuff + 0.25
end
...
if IsValid(Wep:GetHoldType()) and dmginfo:IsDamageType(DMG_SLASH + DMG_CLUB) and table.HasValue(PurifierTable, target:GetClass()) and (table.HasValue(MeleeTypes, Wep:GetHoldType()) or Wep:GetSlot() == 0) then
	DMGBuff = DMGBuff + 0.5
end
end
if PlyHasPerk(Atk, "27 ") == true and IsValid(Wep:GetHoldType()) and ((dmginfo:IsDamageType(DMG_SLASH + DMG_CLUB) and (table.HasValue(MeleeTypes, Wep:GetHoldType()) or Wep:GetSlot() == 0)) or ((dmginfo:GetInflictor():GetClass() == "crossbow_bolt" or dmginfo:GetInflictor():GetClass() == "npc_grenade_frag" or dmginfo:GetInflictor():GetClass() == "rpg_missile") and dmginfo:GetInflictor():GetOwner() == Atk) or table.HasValue(GruntAmmo, dmginfo:GetAmmoType())) then
	DMGBuff = DMGBuff + 0.25
end
...
if PlyHasPerk(target, "22 ") == true and IsValid(Wep:GetHoldType()) and (dmginfo:IsDamageType(DMG_GENERIC + DMG_SLASH + DMG_CLUB)) and (table.HasValue(MeleeTypes, Wep:GetHoldType()) or Wep:GetSlot() == 0) then
	DMGRes = DMGRes - 0.05
end
...
if PlyHasPerk(Atk, "31 ") == true and IsValid(Wep:GetHoldType()) and (dmginfo:IsDamageType(DMG_GENERIC + DMG_SLASH + DMG_CLUB)) and (table.HasValue(MeleeTypes, Wep:GetHoldType()) or Wep:GetSlot() == 0) then
	DMGRes = DMGRes + 0.15
end
