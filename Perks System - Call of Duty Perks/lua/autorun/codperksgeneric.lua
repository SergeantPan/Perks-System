local MechClass = {"npc_manhack", "npc_turret_floor", "npc_cscanner", "npc_combine_camera", "npc_turret_ceiling", "npc_rollermine", "npc_hunter", "npc_strider"}
local MechPartialClass = {"npc_hunter", "npc_strider"}
...
if ply:Alive() and ply:GetNWString("Tier 1 Perk") == "Blind Eye" then
if !table.HasValue(MechPartialClass, npc:GetClass()) and npc:GetNWInt("HackedTeam", -1) != ply:Team() and (npc:Disposition(ply) != D_NU and npc:Disposition(ply) != D_LI) then
	npc:AddEntityRelationship(ply, D_NU, 0)
end

if npc:GetClass() == "npc_hunter" and ply:GetPos():Distance(npc:GetPos()) > 600 and (npc:Disposition(ply) != D_NU and npc:Disposition(ply) != D_LI) then
if npc:GetEnemyLastTimeSeen(ply) + 6 <= CurTime() and (npc:GetLastTimeTookDamageFromEnemy(ply) + 12 <= CurTime() or ply:GetPos():Distance(npc:GetPos()) > 1200) then
	npc:AddEntityRelationship(ply, D_NU, 0)
if npc:GetNPCState() == 3 then
	npc:MarkEnemyAsEluded(ply)
end
end
end

if npc:GetClass() == "npc_strider" and ply:GetPos():Distance(npc:GetPos()) > 1200 and (npc:Disposition(ply) != D_NU and npc:Disposition(ply) != D_LI) then
if npc:GetEnemyLastTimeSeen(ply) + 6 <= CurTime() and (npc:GetLastTimeTookDamageFromEnemy(ply) + 12 <= CurTime() or ply:GetPos():Distance(npc:GetPos()) > 1800) and npc:GetNWBool("HardenedOverride", false) != true then
	npc:AddEntityRelationship(ply, D_NU, 0)
if npc:GetNPCState() == 3 then
	npc:MarkEnemyAsEluded(ply)
end
...
if !ply:Alive() or ply:GetNWString("Tier 1 Perk") != "Blind Eye" then
if npc:GetNWInt("HackedTeam", -1) != ply:Team() and npc:Disposition(ply) != npc.DP then
	npc:AddEntityRelationship(ply, npc.DP, 0)
end
end

if npc:GetClass() == "npc_hunter" and ((npc:GetEnemyLastTimeSeen(ply) + 6 > CurTime() and ply:GetPos():Distance(npc:GetPos()) <= 600) or (npc:GetLastTimeTookDamageFromEnemy(ply) + 12 > CurTime() and ply:GetPos():Distance(npc:GetPos()) <= 1200) or ply:GetPos():Distance(npc:GetPos()) <= 600) and npc:Disposition(ply) != npc.DP then
	npc:AddEntityRelationship(ply, npc.DP, 0)
end
if npc:GetClass() == "npc_strider" and ((npc:GetEnemyLastTimeSeen(ply) + 6 > CurTime() and ply:GetPos():Distance(npc:GetPos()) <= 1200) or (npc:GetLastTimeTookDamageFromEnemy(ply) + 12 > CurTime() and ply:GetPos():Distance(npc:GetPos()) <= 1800) or ply:GetPos():Distance(npc:GetPos()) <= 1200 or npc:GetNWBool("HardenedOverride", false) == true) and npc:Disposition(ply) != npc.DP then
	npc:AddEntityRelationship(ply, npc.DP, 0)
end
end
