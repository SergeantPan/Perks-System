...
for _,Mechs in pairs(ents.FindByClass("npc_*")) do
if table.HasValue(MechClass, Mechs:GetClass()) and Mechs:GetPos():Distance(LocalPlayer():GetPos()) < 300 then
cam.Start3D()
Mechs:DrawModel()
cam.End3D()
end
...
for _,SitrepGren in pairs(ents.FindByClass("npc_grenade_frag")) do
if SitrepGren:GetPos():Distance(LocalPlayer():GetPos()) < 300 then
cam.Start3D()
SitrepGren:DrawModel()
cam.End3D()
end
end
end
...
if LocalPlayer():GetNWString("Tier 3 Perk") == "SitRep" then
for _,SitrepSlam in pairs(ents.FindByClass("weapon_slam")) do
if SitrepSlam:GetPos():Distance(LocalPlayer():GetPos()) < 300 then
cam.Start3D()
SitrepSlam:DrawModel()
cam.End3D()
end
end
end
...
hook.Add( "PreDrawHalos", "PulsarHalos", function()
...
hook.Add( "PreDrawHalos", "HackerHalos", function()
...
hook.Add( "PreDrawHalos", "ReconHalos", function()
local targets = {}
local mechs = {}
local explosives = {}
...
for _,Slams in pairs(ents.FindByClass("weapon_slam")) do
